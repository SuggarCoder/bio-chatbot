...
CREATE TABLE "Artifact" (
	...
	CONSTRAINT "chk_artifact_format" CHECK ("Artifact"."format" in ('html', 'markdown', 'pdf', 'csv', 'xlsx', 'json', 'png', 'jpeg', 'svg')),
	CONSTRAINT "chk_artifact_status" CHECK ("Artifact"."status" in ('generating', 'ready', 'failed', 'expired')),
...
CREATE TABLE "Generation" (
	...
	"updatedAt" timestamp with time zone DEFAULT now() NOT NULL,
	"finishedAt" timestamp with time zone,
	CONSTRAINT "uq_generation_request_id" UNIQUE("requestId"),
	CONSTRAINT "chk_generation_status" CHECK ("Generation"."status" in ('pending', 'streaming', 'completed', 'failed', 'cancelled')),
	...
	CONSTRAINT "chk_generation_finished_at" CHECK (("Generation"."status" in ('completed', 'failed', 'cancelled') and "Generation"."finishedAt" is not null)
        or ("Generation"."status" in ('pending', 'streaming') and "Generation"."finishedAt" is null))
);
--> statement-breakpoint
...
CREATE TABLE "Message" (
	...
	CONSTRAINT "chk_message_status" CHECK ((
        ("Message"."role" = 'assistant' and "Message"."status" in ('completed', 'cancelled', 'failed'))
...
CREATE TABLE "Stream" (
	...
	"createdAt" timestamp with time zone DEFAULT now() NOT NULL,
	"expiresAt" timestamp with time zone DEFAULT now() + interval '24 hours' NOT NULL,
	CONSTRAINT "chk_stream_expiry" CHECK ("Stream"."expiresAt" > "Stream"."createdAt")
);
--> statement-breakpoint
...
ALTER TABLE "AnalysisJob" ADD CONSTRAINT "AnalysisJob_userId_User_id_fk" FOREIGN KEY ("userId") REFERENCES "public"."User"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "Artifact" ADD CONSTRAINT "Artifact_userId_User_id_fk" FOREIGN KEY ("userId") REFERENCES "public"."User"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "Generation" ADD CONSTRAINT "Generation_chatId_Chat_id_fk" FOREIGN KEY ("chatId") REFERENCES "public"."Chat"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "Generation" ADD CONSTRAINT "Generation_userId_User_id_fk" FOREIGN KEY ("userId") REFERENCES "public"."User"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "Generation" ADD CONSTRAINT "Generation_userMessageId_Message_id_fk" FOREIGN KEY ("userMessageId") REFERENCES "public"."Message"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "Generation" ADD CONSTRAINT "Generation_assistantMessageId_Message_id_fk" FOREIGN KEY ("assistantMessageId") REFERENCES "public"."Message"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "Stream" ADD CONSTRAINT "Stream_generationId_Generation_id_fk" FOREIGN KEY ("generationId") REFERENCES "public"."Generation"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "uq_generation_assistant_message" ON "Generation" USING btree ("assistantMessageId") WHERE "Generation"."assistantMessageId" is not null;--> statement-breakpoint
CREATE UNIQUE INDEX "uq_generation_chat_active" ON "Generation" USING btree ("chatId") WHERE "Generation"."chatId" is not null and "Generation"."status" in ('pending', 'streaming')
          and "Generation"."cancelRequestedAt" is null;--> statement-breakpoint
...
CREATE UNIQUE INDEX "uq_stream_generation" ON "Stream" USING btree ("generationId");--> statement-breakpoint
CREATE INDEX "idx_stream_expiry" ON "Stream" USING btree ("expiresAt");--> statement-breakpoint
