...
export const messages = pgTable(
  ...
  (table) => [
    ...
    check(
      'chk_message_status',
      sql`(
        (${table.role} = 'assistant' and ${table.status} in ('completed', 'cancelled', 'failed'))
...
export const generations = pgTable(
  'Generation',
  {
    ...
    userMessageId: uuid('userMessageId').references(() => messages.id, {
      onDelete: 'set null',
    }),
    assistantMessageId: uuid('assistantMessageId').references(() => messages.id, {
      onDelete: 'set null',
    }),
  ...
  (table) => [
    unique('uq_generation_request_id').on(table.requestId),
    check(
    ...
    check(
      ...
      sql`(${table.status} in ('completed', 'failed', 'cancelled') and ${table.finishedAt} is not null)
        or (${table.status} in ('pending', 'streaming') and ${table.finishedAt} is null)`,
    ),
    ...
    uniqueIndex('uq_generation_chat_active')
      .on(table.chatId)
      .where(
        sql`${table.chatId} is not null and ${table.status} in ('pending', 'streaming')
...
export const streams = pgTable(
  ...
  (table) => [
    check('chk_stream_expiry', sql`${table.expiresAt} > ${table.createdAt}`),
    uniqueIndex('uq_stream_generation').on(table.generationId),
    index('idx_stream_expiry').on(table.expiresAt),
  ],
)
